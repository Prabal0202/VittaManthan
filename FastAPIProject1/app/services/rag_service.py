...
YOUR DETAILED ENGLISH RESPONSE:""")
    ...
    async def process_analytical_query_stream(
        self,
        documents: List[Dict],
        prompt: str
    ) -> AsyncGenerator[str, None]:
        """Process analytical queries with streaming response"""
        logger.info(f"Analytical query detected (streaming) - analyzing ALL {len(documents)} transactions")
...
🚀 YOUR INTELLIGENT RESPONSE (analyzing ALL {total_transactions} transactions):""")
    ...
    async def process_vector_search_query_stream(
        self,
        vectorstore: FAISS,
        prompt: str,
        k_value: int = 50
    ) -> AsyncGenerator[str, None]:
        """Process specific queries using vector similarity search with streaming"""
        logger.info(f"Using vector similarity search with k={k_value} (streaming)")
...
YOUR DETAILED ENGLISH RESPONSE:""")
    ...
    async def process_smart_full_query_stream(
        self,
        documents: List[Dict],
        prompt: str
    ) -> AsyncGenerator[str, None]:
        """Process full scan queries with streaming response"""
        from app.utils.answer_generator import generate_conversational_answer_stream
        
        filters = extract_filters_from_query(prompt)
        filtered_docs, filter_descriptions = apply_filters(documents, filters, prompt)

        # Stream the answer using LLM
        async for chunk in generate_conversational_answer_stream(
            prompt,
            filtered_docs,
            filters,
            filter_descriptions,
